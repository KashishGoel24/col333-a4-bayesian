...
int main(int argc, char *argv[])
{
    ...
    int total_time_in_sec = 40 ; 
    int last_end_time = time(NULL) ;
    float score;
    int total_iters = 0 ;
    int fraction_data = 2 ; // idea 4
    int update_every = 30 ;  // idea 2
    int dont_update = 50 ; // idea 3
    ...
    for (int all_data = 0 ; all_data < data.size() ; all_data++){
        ...
        if (all_data==0) vec_print(data_row) ;
        ...
        if (not question_mark){
            update_cpt(weight_sampling,&Alarm,value_row, (total_iters < (data.size()/fraction_data) || (total_iters%update_every == 0))) ;
        }
        else{
        ...
        for (int i=0  ; i < nvals_x ; i++){
            ...
            else update_cpt(prob_table_x[i] * weight_sampling,&Alarm , value_row , (total_iters < (data.size()/fraction_data) || (total_iters%update_every == 0))) ;
